...
let now=1000;
const toasts=[];
class FakeImage{set src(value){this._src=value;this.complete=true;this.naturalWidth=320;queueMicrotask(()=>this.onload?.());}get src(){return this._src;}}
const ctx={imageSmoothingEnabled:true,fillStyle:'',font:'',textAlign:'',globalAlpha:1,save(){},restore(){},drawImage(){drawImages++;},fillRect(){fillRects++;},fillText(){}};
const CAMP={x:2400,y:2400,r:470,inner:330};
const sandbox={
  console,window:null,Image:FakeImage,queueMicrotask,performance:{now:()=>now},CAMP,
  camera:{x:2400,y:1280},canvas:{width:360,height:600},ctx,currentZone:'1:0',started:true,dead:false,hasLeftCamp:true,
  me:{x:2400,y:1280,r:14},inventory:{wood:4,stone:3,food:1,shard:0,knife:true,lantern:false},resources:[],mobs:[],questText:{textContent:''},
  toast(text){toasts.push(String(text));},drawGround(){groundCalls++;},drawCamp(){campCalls++;},drawLighting(){lightingCalls++;},update(){},sendMove(){},
...
sandbox.seedZone=zone=>{sandbox.resources.length=0;if(String(zone)==='1:0')sandbox.resources.push({id:'1:0:legacy-inside',type:'stone',x:2570,y:817,r:18},{id:'1:0:legacy-roadside',type:'wood',x:1900,y:1500,r:18});};
...
assert.equal(api.version,1);assert.equal(api.placeholderArt,true,'programmer art must be explicitly marked temporary');assert.equal(api.artContract,'AWAKENING_ASSET_CONTRACT.md');
assert.equal(api.stage.stage,0,'slice must begin in normal-reality Stage 0');assert.equal(api.stage.earlyStage1,true,'only a very early Stage 1 hint is allowed');assert.ok(api.stage.corruptionIntensity>0&&api.stage.corruptionIntensity<=.1,'early corruption must remain subtle');
...
assert.equal(poiResources.length,6,'store must seed six deterministic search/loot nodes');assert.equal(poiResources.filter(r=>r.type==='shard').length,2,'POI must provide two high-value shard rewards for next-action progression');assert.ok(poiResources.filter(r=>r.type==='food').length>=3,'store must provide ordinary survival supplies');assert.ok(poiResources.every(r=>['wood','stone','food','shard'].includes(r.type)),'POI loot must reuse the existing inventory schema');assert.ok(poiResources.every(r=>r.poi===store.id),'loot nodes must remain attributable to the POI');
assert.equal(sandbox.resources.some(r=>r.id==='1:0:legacy-inside'),false,'legacy random resources inside the authored POI must be cleared to preserve composition and reachability');assert.equal(sandbox.resources.some(r=>r.id==='1:0:legacy-roadside'),true,'legacy resources outside the POI clear area must remain available');
sandbox.seedZone('1:1');assert.equal(sandbox.resources.some(r=>String(r.id).startsWith('poi:store:')),false,'POI loot must not leak into Safe Camp zone seeding');

function block(text,startToken,endToken){const start=text.indexOf(startToken),end=text.indexOf(endToken,start);assert.ok(start>=0&&end>start,`missing source block ${startToken}`);return text.slice(start,end);}
const harvestSource=block(interaction,'function harvestResource(r){','function restAtFire(){');const sample={...api.layers.interactions.find(v=>v.type==='shard')};const harvestCalls=[];
const lootSandbox={resource:sample,harvested:new Map(),inventory:{wood:0,stone:0,food:0,shard:0},ITEM_ZH:{wood:'木材',stone:'石头',food:'食物',shard:'异质碎片'},zoneConnected:true,currentZone:'1:0',zoneCh:{send(message){harvestCalls.push(message);return Promise.resolve('ok');}},saveLocal(){},updateUI(){},toast(){}};
vm.createContext(lootSandbox);vm.runInContext(`${harvestSource}\nharvestResource(resource);`,lootSandbox,{filename:'poi-loot-smoke.js'});
assert.equal(lootSandbox.inventory.shard,1,'existing context harvest must award injected POI shard loot');assert.ok(lootSandbox.harvested.get(sample.id)>Date.now(),'POI loot must enter existing harvested cooldown state');assert.equal(harvestCalls.length,1,'non-camp POI loot must use the existing shared harvest broadcast path');assert.equal(harvestCalls[0].event,'harvest');assert.equal(harvestCalls[0].payload.rid,sample.id);

sandbox.spawnMobs('1:0');assert.equal(sandbox.mobs.filter(m=>String(m.id).startsWith('poi:store:crawler-')).length,2,'store encounter must seed two deliberate crawlers');assert.ok(sandbox.mobs.length<=10,'POI encounter must remain within existing inbound mob cap');

sandbox.currentZone='1:1';sandbox.me.x=2400;sandbox.me.y=1880;sandbox.inventory.shard=0;sandbox.updateQuest();assert.ok(/北门公路|便利店/.test(sandbox.questText.textContent),'HOME must point the prepared player toward a clear north-road objective');
sandbox.currentZone='1:0';sandbox.me.x=2400;sandbox.me.y=850;sandbox.updateQuest();assert.ok(/便利店/.test(sandbox.questText.textContent)&&/搜索/.test(sandbox.questText.textContent),'inside POI objective must tell player to search the environment');sandbox.inventory.shard=2;sandbox.updateQuest();assert.ok(/返回 Safe Camp/.test(sandbox.questText.textContent),'valuable POI reward must create a return-home objective');sandbox.currentZone='1:1';sandbox.me.x=CAMP.x;sandbox.me.y=CAMP.y;sandbox.updateQuest();assert.ok(/灯笼|下一次外出/.test(sandbox.questText.textContent),'returning HOME with shards must point toward the next preparation action');

...
function drawStoreFloor(){
  if(artApi?.hasBinding?.('terrain','store_floor')){for(let y=STORE.y;y<STORE.y+STORE.h;y+=TILE_SIZE)for(let x=STORE.x;x<STORE.x+STORE.w;x+=TILE_SIZE)artApi.drawTerrain('store_floor',x,y,Math.floor(hash2(x/TILE_SIZE,y/TILE_SIZE,44)*1e6));return;}
...
function drawTerrainLayer(W,H){if(!rectIntersectsView(REGION,W,H))return;ctx.save();ctx.imageSmoothingEnabled=false;const startX=Math.floor(REGION.x/TILE_SIZE)*TILE_SIZE,startY=Math.floor(REGION.y/TILE_SIZE)*TILE_SIZE,endX=REGION.x+REGION.w,endY=REGION.y+REGION.h;for(let wy=startY;wy<endY;wy+=TILE_SIZE)for(let wx=startX;wx<endX;wx+=TILE_SIZE){const material=regionMaterial(wx,wy);if(material==='store_floor')continue;const seed=Math.floor(hash2(wx/TILE_SIZE,wy/TILE_SIZE,913)*1e6);if(!artApi?.drawTerrain?.(material,wx,wy,seed))drawAtlasTile(tileForMaterial(material,wx,wy),wx,wy);const cold=clamp01((1900-(wy+32))/1050);if(cold>0){ctx.fillStyle=`rgba(38,55,65,${(.08+cold*.16).toFixed(3)})`;ctx.fillRect(sx(wx),sy(wy),TILE_SIZE,TILE_SIZE);}}drawStoreFloor();ctx.restore();}
...
window.ABYSSAL_AWAKENING_WORLD_V1={version:VERSION,stage:WORLD_STAGE,region:REGION,poi:STORE,layers:LAYERS,placeholderArt:true,artContract:'AWAKENING_ASSET_CONTRACT.md',presentation:artApi,roadCenterAt,regionMaterial,inStore,isBlockedPoint,isBlockedCircle,resolveMovement,addPoiResources,objectiveText};
})();

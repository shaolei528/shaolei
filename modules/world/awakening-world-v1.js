...
/*
  ...
  This extends the existing V21 world in world coordinates without changing
  Relay protocol, save schema, combat authority, or the Safe Camp rules.
*/
...
const STORE={
  id:'poi:abandoned-convenience-store',
  label:'Abandoned Convenience Store',
...
const WORLD_STAGE={
  ...
  stage:0,
  earlyStage1:true,
  corruptionIntensity:.08,
  rule:'local-authored-overlays-only'
};

const terrainApi=window.ABYSSAL_TERRAIN_V21||null;
const TILE=terrainApi?.tile||{
...
const DECALS=[
  {id:'puddle-1',kind:'puddle',x:2336,y:1744,w:72,h:26},
  {id:'puddle-2',kind:'puddle',x:2472,y:1572,w:52,h:20},
  {id:'puddle-3',kind:'puddle',x:2232,y:1324,w:68,h:24},
  {id:'crack-1',kind:'crack',x:2412,y:1816,w:72,h:40},
  {id:'crack-2',kind:'crack',x:2356,y:1492,w:84,h:44},
  {id:'crack-3',kind:'crack',x:2488,y:1250,w:62,h:34},
  {id:'tire-1',kind:'tire',x:2384,y:1650,w:10,h:96},
  {id:'tire-2',kind:'tire',x:2472,y:1650,w:10,h:96},
  {id:'store-glass',kind:'glass',x:2440,y:1128,w:76,h:38},
  {id:'residue-1',kind:'residue',x:2624,y:950,w:84,h:54}
];

const PROPS=[
  {id:'fence-west-a',kind:'fence',x:2024,y:1192,w:16,h:176},
  {id:'fence-east-a',kind:'fence',x:2824,y:1208,w:16,h:160},
  {id:'barrier-left',kind:'barrier',x:2080,y:1392,w:96,h:24},
  {id:'barrier-right',kind:'barrier',x:2688,y:1456,w:96,h:24},
  {id:'store-wall-n',kind:'wall',x:STORE.x,y:STORE.y,w:STORE.w,h:16},
  {id:'store-wall-w',kind:'wall',x:STORE.x,y:STORE.y,w:16,h:STORE.h},
  {id:'store-wall-e',kind:'wall',x:STORE.x+STORE.w-16,y:STORE.y,w:16,h:STORE.h},
  {id:'store-wall-s-left',kind:'wall',x:STORE.x,y:STORE.y+STORE.h-16,w:280,h:16},
  {id:'store-wall-s-right',kind:'wall',x:STORE.entrance.x+STORE.entrance.w,y:STORE.y+STORE.h-16,w:280,h:16},
  {id:'store-counter',kind:'counter',x:2160,y:704,w:160,h:48},
  {id:'store-shelf-a',kind:'shelf',x:2288,y:800,w:128,h:32},
  {id:'store-shelf-b',kind:'shelf',x:2464,y:800,w:128,h:32},
  {id:'store-shelf-c',kind:'shelf',x:2288,y:880,w:128,h:32},
  {id:'store-fridges',kind:'fridge',x:2584,y:688,w:112,h:48},
  {id:'store-debris',kind:'debris',x:2504,y:936,w:80,h:56}
];
...
const LAYERS={
  terrain:{region:REGION,store:STORE,tileSize:TILE_SIZE},
  decals:DECALS,
  props:PROPS,
  collision:COLLISION,
...
function rectIntersectsView(r,W,H){
  const left=camera.x-W/2,right=camera.x+W/2,top=camera.y-H/2,bottom=camera.y+H/2;
  return right>=r.x&&left<=r.x+r.w&&bottom>=r.y&&top<=r.y+r.h;
}
...
function roadCenterAt(y){
  if(y>1720)return 2400;
  if(y>1480)return 2368;
  if(y>1240)return 2400;
  return 2432;
}
function regionMaterial(wx,wy){
  if(inRect(wx+32,wy+32,STORE))return'store_floor';
  if(inRect(wx+32,wy+32,STORE.parking))return'asphalt';
  const center=roadCenterAt(wy+32),dx=Math.abs((wx+32)-center);
  if(dx<=96)return'asphalt';
  if(dx<=224)return'dirt_shoulder';
  return'cold_grass';
}
function tileForMaterial(material,wx,wy){
  const h=hash2(Math.floor(wx/TILE_SIZE),Math.floor(wy/TILE_SIZE),913);
  if(material==='asphalt')return h>.55?TILE.road_b:TILE.road_a;
  if(material==='dirt_shoulder')return h>.58?TILE.sand_b:TILE.sand_a;
  if(material==='cold_grass')return h>.88?TILE.grass_flowers:(h>.48?TILE.grass_b:TILE.grass_a);
  return null;
}
...
function drawAtlasTile(tile,wx,wy){
  const x=sx(wx),y=sy(wy);
  if(sheetReady&&sheet&&Number.isFinite(tile)){
    const srcX=(tile%SHEET_COLS)*TILE_SIZE,srcY=Math.floor(tile/SHEET_COLS)*TILE_SIZE;
    ctx.drawImage(sheet,srcX,srcY,TILE_SIZE,TILE_SIZE,x,y,TILE_SIZE,TILE_SIZE);
    return;
  }
  ctx.fillStyle=tile===TILE.road_a||tile===TILE.road_b?'#444d50':(tile===TILE.sand_a||tile===TILE.sand_b?'#5d5b50':'#4d6259');
  ctx.fillRect(x,y,TILE_SIZE,TILE_SIZE);
}
function drawStoreFloor(){
  const r=screenRect(STORE);
  ctx.fillStyle='#343c40';ctx.fillRect(r.x,r.y,r.w,r.h);
  ctx.fillStyle='#3c4549';
  for(let y=STORE.y+16;y<STORE.y+STORE.h-16;y+=32){
    for(let x=STORE.x+16;x<STORE.x+STORE.w-16;x+=32){
      if(((x+y)>>5)&1)ctx.fillRect(sx(x),sy(y),32,32);
    }
  }
  ctx.fillStyle='rgba(118,142,148,.08)';ctx.fillRect(r.x,r.y,r.w,r.h);
}
function drawTerrainLayer(W,H){
  if(!rectIntersectsView(REGION,W,H))return;
  ctx.save();ctx.imageSmoothingEnabled=false;
  const startX=Math.floor(REGION.x/TILE_SIZE)*TILE_SIZE,startY=Math.floor(REGION.y/TILE_SIZE)*TILE_SIZE;
  const endX=REGION.x+REGION.w,endY=REGION.y+REGION.h;
  for(let wy=startY;wy<endY;wy+=TILE_SIZE){
    for(let wx=startX;wx<endX;wx+=TILE_SIZE){
      const material=regionMaterial(wx,wy);
      if(material==='store_floor')continue;
      drawAtlasTile(tileForMaterial(material,wx,wy),wx,wy);
      const cold=clamp01((1900-(wy+32))/1050);
      if(cold>0){ctx.fillStyle=`rgba(38,55,65,${(.08+cold*.16).toFixed(3)})`;ctx.fillRect(sx(wx),sy(wy),TILE_SIZE,TILE_SIZE);}
    }
  }
  drawStoreFloor();
  ctx.restore();
...
function drawCrack(d){
  const x=sx(d.x),y=sy(d.y);ctx.fillStyle='#263034';
  ctx.fillRect(x,y+8,22,3);ctx.fillRect(x+18,y+10,3,12);ctx.fillRect(x+20,y+19,28,3);ctx.fillRect(x+44,y+20,3,12);ctx.fillRect(x+46,y+29,20,3);
}
function drawPuddle(d){
  const x=sx(d.x),y=sy(d.y);ctx.fillStyle='rgba(77,101,110,.45)';
  ctx.fillRect(x+8,y,d.w-16,d.h);ctx.fillRect(x,y+6,d.w,d.h-12);ctx.fillStyle='rgba(148,169,173,.18)';ctx.fillRect(x+14,y+5,Math.max(8,d.w-34),3);
}
function drawDecalLayer(W,H){
  ctx.save();ctx.imageSmoothingEnabled=false;
  for(const d of DECALS){
    if(Math.abs(sx(d.x))>W+180||Math.abs(sy(d.y))>H+180)continue;
    if(d.kind==='puddle')drawPuddle(d);
    else if(d.kind==='crack')drawCrack(d);
    else if(d.kind==='tire'){ctx.fillStyle='rgba(29,36,39,.30)';ctx.fillRect(sx(d.x),sy(d.y),d.w,d.h);}
    else if(d.kind==='glass'){const x=sx(d.x),y=sy(d.y);ctx.fillStyle='rgba(146,170,176,.35)';for(let i=0;i<7;i++)ctx.fillRect(x+(i*11)%d.w,y+(i*7)%d.h,3,2);}
    else if(d.kind==='residue'){const x=sx(d.x),y=sy(d.y);ctx.fillStyle='rgba(98,133,127,.26)';ctx.fillRect(x+10,y+18,56,8);ctx.fillRect(x+24,y+9,30,24);ctx.fillStyle='rgba(153,185,176,.18)';ctx.fillRect(x+34,y+2,12,42);}
  }
  ctx.restore();
}
...
function drawPropLayer(W,H){
  ctx.save();ctx.imageSmoothingEnabled=false;
  for(const p of PROPS){
    const r=screenRect(p);if(r.x>W+80||r.y>H+80||r.x+r.w<-80||r.y+r.h<-80)continue;
    if(p.kind==='wall')drawWall(p);else if(p.kind==='shelf')drawShelf(p);else if(p.kind==='fridge')drawFridge(p);else if(p.kind==='counter')drawCounter(p);else if(p.kind==='debris')drawDebris(p);else if(p.kind==='fence')drawFence(p);else if(p.kind==='barrier')drawBarrier(p);
  }
  if(rectIntersectsView(STORE,W,H)){
    ctx.fillStyle='#d3c8aa';ctx.font='bold 8px monospace';ctx.textAlign='center';ctx.fillText('MIRE MART',sx(STORE.x+STORE.w/2),sy(STORE.y+STORE.h+20));
    ctx.fillStyle='#829098';ctx.font='bold 6px monospace';ctx.fillText('NO POWER',sx(STORE.x+STORE.w/2),sy(STORE.y+STORE.h+31));
  }
  ctx.restore();
}
function drawFxLayer(W,H){
  if(!rectIntersectsView(STORE,W,H))return;
  const t=typeof performance!=='undefined'?performance.now():0;
  const pulse=.16+(Math.sin(t/173)+1)*.05;
  ctx.save();ctx.imageSmoothingEnabled=false;
  ctx.fillStyle=`rgba(143,181,174,${pulse.toFixed(3)})`;ctx.fillRect(sx(2624),sy(950),84,5);ctx.fillRect(sx(2650),sy(934),28,28);
  if((Math.floor(t/380)&3)!==1){ctx.fillStyle='rgba(189,207,199,.22)';ctx.fillRect(sx(2610),sy(676),96,4);}
  ctx.restore();
}
...
function isBlockedCircle(x,y,radius=10){
  ...
  for(let i=0;i<8;i++){const a=i*Math.PI/4;if(isBlockedPoint(x+Math.cos(a)*r,y+Math.sin(a)*r))return true;}
  return false;
}
function resolveMovement(fromX,fromY,toX,toY,radius=10){
  ...
  const dist=Math.hypot(tx-fx,ty-fy),steps=Math.max(1,Math.ceil(dist/8));let blocked=false;
  for(let i=1;i<=steps;i++){
    const wantX=fx+(tx-fx)*(i/steps),wantY=fy+(ty-fy)*(i/steps);
    if(!isBlockedCircle(wantX,wantY,radius)){x=wantX;y=wantY;continue;}
    blocked=true;if(!isBlockedCircle(wantX,y,radius))x=wantX;if(!isBlockedCircle(x,wantY,radius))y=wantY;
  }
  return{x,y,blocked};
}
...
function addPoiResources(zone){
  ...
  for(const def of INTERACTIONS){
    if(resources.some(r=>r.id===def.id))continue;
    resources.push({id:def.id,type:def.type,x:def.x,y:def.y,r:def.r,poi:STORE.id,source:def.source});
  }
}
...
if(baseSeedZone){
  window.seedZone=seedZone=function(zone){const result=baseSeedZone(zone);addPoiResources(zone);return result;};
}
const baseFreshMob=typeof window.freshMob==='function'?window.freshMob:null;
if(baseFreshMob){
  window.freshMob=freshMob=function(zone,index){
    const def=ENTITIES.find(e=>String(zone)==='1:0'&&e.index===index);
    return def?makePoiMob(def):baseFreshMob(zone,index);
  };
}
const baseSpawnMobs=typeof window.spawnMobs==='function'?window.spawnMobs:null;
if(baseSpawnMobs){
  window.spawnMobs=spawnMobs=function(zone){
    baseSpawnMobs(zone);
    if(String(zone)==='1:0')for(const def of ENTITIES)if(!mobs.some(m=>m.id===def.id))mobs.push(makePoiMob(def));
  };
}
...
if(baseSendMove){
  window.sendMove=function(...args){
    if(movementOrigin&&typeof me!=='undefined'){
      const resolved=resolveMovement(movementOrigin.x,movementOrigin.y,me.x,me.y,Math.min(12,me.r||10));me.x=resolved.x;me.y=resolved.y;
    }
...
function objectiveText(){
  ...
  if(!inventory.knife)return null;
  if(inCamp()){
    if(hasLeftCamp&&inventory.shard>=2){
  ...
  if(inStore()){
    if(inventory.shard>=2)return '异常残留已取得 · 带着补给返回 Safe Camp。';
    return '便利店：搜索货架、冰柜、碎屑与异常残留。';
  }
  if(inventory.shard>=2)return '带着便利店补给返回 Safe Camp。';
  return '沿破损北路前进 · 寻找停电便利店。';
}
if(baseUpdateQuest){
  window.updateQuest=updateQuest=function(){baseUpdateQuest();const text=objectiveText();if(text&&questText)questText.textContent=text;};
}
...
const API={
  ...
  layers:LAYERS,
  placeholderArt:true,
  artContract:'AWAKENING_ASSET_CONTRACT.md',
  roadCenterAt,regionMaterial,inStore,isBlockedPoint,isBlockedCircle,resolveMovement,
  addPoiResources,objectiveText
};
window.ABYSSAL_AWAKENING_WORLD_V1=API;
})();